...
module.exports.sockets = {
  ...
  ***************************************************************************/
  afterDisconnect: function(session, socket, cb) {
    console.log('fired afterDisconnect w/ session.me=',session.me,'\n(socket id = '+socket.id+')');

    // Determine the id of the disconnecting user
    var disconnectingUserId = (function _determineDisconnectingUserId (){
      ...
      // (we have to do this because the session might be gone before this
      //  `afterDisconnect` socket lifecycle callback fires.)
      if (session.previousMe){
        return session.previousMe;
      }
    })();

    // If the disconnecting socket (e.g. tab) is not currently (and wasn't previously)
    // logged in, we don't care-- this must be someone who has never logged in with
    // this session. We'll just get out of here in that case b/c we don't know which (if any)
    // User they correspond to.
    ...
    User.findOne(disconnectingUserId).exec(function (err, user) {
      if (err) return cb(err);
      if (!user) {
        return cb(new Error('User associated with disconnecting socket no longer exists.'));
      }

      // This user will momentarily be considered "offline", since she
      // will have exactly zero sockets connected (e.g. browser tabs open)
      var isLastSocket = user.numSocketsConnected === 1;

      User.update(user.id, {
        numSocketsConnected: (function _determineNewNumSocketsConnected(){
          // The ternary operator is just here to ensure that we never drop below 0.
          return user.numSocketsConnected-1 < 0 ? 0 : user.numSocketsConnected-1;
        })()
      }).exec(function (err){
        if (err) return cb(err);
        ...
        if (isLastSocket) {
          ...
          User.publishUpdate(disconnectingUserId, {
